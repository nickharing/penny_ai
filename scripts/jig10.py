...
class CoinProcessor:
    def __init__(self, image_path, max_display_size=920):
        ...
        print(f"Image: {self.image_path} ({self.original_image.shape[1]}×{self.original_image.shape[0]})")
        print(f"Display Size: {self.display_image.shape[1]}×{self.display_image.shape[0]} (Scale: {self.display_scale:.3f})")
        print(f"------------------------------")
    ...
    def extract_coin(self):
        """Adjust circle on warped image and extract the circular coin."""
        if self.warped_image is None:
            print("Please correct warp first")
            return False
        
        disp_w, scale = self.resize_for_display(self.warped_image)
        win, needs_redraw = "Adjust Coin", True
        
        def draw_dashed_circle(img, center, radius):
            segments = 60
            step = 360 / segments
            for i in range(segments):
                sa = np.deg2rad(i*step)
                ea = np.deg2rad(i*step + step/2)
                p1 = (int(center[0]+radius*np.cos(sa)), int(center[1]+radius*np.sin(sa)))
                p2 = (int(center[0]+radius*np.cos(ea)), int(center[1]+radius*np.sin(ea)))
                cv2.line(img, p1, p2, (0,255,255),2)
        
        key_actions = {
            2490368: lambda: self.move_circle(0, -self.POSITION_CHANGE),   # Up
            2621440: lambda: self.move_circle(0, self.POSITION_CHANGE),    # Down
            2424832: lambda: self.move_circle(-self.POSITION_CHANGE, 0),   # Left
            2555904: lambda: self.move_circle(self.POSITION_CHANGE, 0),    # Right
        ...
        while True:
            if needs_redraw:
                img = disp_w.copy()
                cc = (int(self.circle_center[0]*scale), int(self.circle_center[1]*scale))
                cr = int(self.circle_radius*scale)
                
                draw_dashed_circle(img, cc, cr)
                ang = np.deg2rad(self.rotation_angle - 90)  # 12 o'clock base
                end_pt = (int(cc[0] + cr*np.cos(ang)), int(cc[1] + cr*np.sin(ang)))
                cv2.line(img, cc, end_pt, (0,255,255),1)
                
                y0 = 30
                for txt in ["Arrows: Move", "+/-: Resize", "1/2: Rotate", "Enter: Confirm", "Esc: Cancel"]:
                    cv2.putText(img, txt, (10, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),1)
                    y0 += 20
                status = f"C: {self.circle_center} R: {self.circle_radius} A: {self.rotation_angle}°"
                cv2.putText(img, status, (10, img.shape[0]-20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),1)
        ...
        cv2.circle(mask, (self.circle_radius, self.circle_radius), self.circle_radius, 255, -1)
    ...
    def define_rois(self):
        """Let user draw 3 ROIs on the extracted coin."""
        ...
        for name in ['date', 'mint_mark', 'liberty']:
            ...
            def on_mouse(evt, x, y, flags, param):
                ...
                if evt == cv2.EVENT_LBUTTONDOWN:
                    start = (x, y); end = (x, y); drawing = True; needs_redraw = True
                elif evt == cv2.EVENT_MOUSEMOVE and drawing:
                    end = (x, y); needs_redraw = True
                elif evt == cv2.EVENT_LBUTTONUP and drawing:
                    end = (x, y); drawing = False; needs_redraw = True
...
def main():
    ...
    out_json = path.split('.')[0] + "_data.json"
    
    proc = CoinProcessor(path, max_display_size=max_sz)
    if not proc.correct_warp():
        return
    
    samples = []
    for i in range(10):
        print(f"\n=== Coin {i+1} of 10 ===")
        if not proc.extract_coin():
            print("Extraction cancelled.")
            break
        if not proc.define_rois():
            print("ROI selection cancelled.")
            break
        
        diameter_px = 2 * proc.circle_radius
        sample = {
            'diameter_px': diameter_px,
            'center': proc.circle_center,
            'roi_coordinates': proc.rois.copy(),
            'normalized_roi_coordinates': proc.normalized_rois.copy()
        }
        samples.append(sample)
    
    # compute averages
    if samples:
        avg_diameter = sum(s['diameter_px'] for s in samples) / len(samples)
        avg_norm = { name: [0,0,0,0] for name in proc.rois }
        for s in samples:
            for name, vals in s['normalized_roi_coordinates'].items():
                for j,v in enumerate(vals):
                    avg_norm[name][j] += v
        avg_norm = { name: tuple(vals/len(samples) for vals in [avg_norm[name][j] for j in range(4)] )
                     if False else tuple(v/len(samples) for v in avg_norm[name])
                     for name in avg_norm }
        
        data = {
            'samples': samples,
            'average_diameter_px': avg_diameter,
            'average_normalized_rois': avg_norm
        }
