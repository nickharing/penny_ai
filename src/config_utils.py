...
class TrainingConfig:
    ...
    # -------------------------------------------------------------- new param
    img_size:   int       = 224                   # pixels, will drive Resize
...
def load_config(
    ...
    *,
    ...
    """
    Load the YAML at `config_path`, pick the `model_key` section,
    expand paths, and slam into the TrainingConfig dataclass.
    """
    with open(config_path, "r", encoding="utf-8") as fh:
        full_yaml: Dict[str, Any] = yaml.safe_load(fh)

    if model_key not in full_yaml:
        raise KeyError(f"model_key '{model_key}' not found in {config_path}")

    cfg_dict = full_yaml[model_key]

    # Expand path‐strings → Path objects
